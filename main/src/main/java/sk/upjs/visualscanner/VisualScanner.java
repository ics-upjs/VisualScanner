/**
 * This code is an extension of the original Scanner class from Java 6.
 */
...
/**
 * Scanner that extends the original Java 6 Scanner with ability to visualize
 * its current state.
 */
public final class VisualScanner implements Iterator<String> {
	...
	private void buildFloatAndDecimalPattern() {
		...
		String numeral = "((" + digit + "++)|" + groupedNumeral + ")";
		String decimalNumeral = "(" + numeral + "|" + numeral + decimalSeparator + digit + "*+|" + decimalSeparator
				+ digit + "++)";
	...
	/**
	 * Constructs a <code>Scanner</code> that returns values scanned from the
	 * specified source delimited by the specified pattern.
	 * 
	 * @param source
	 *            A character source implementing the Readable interface
	 * @param pattern
	 *            A delimiting pattern
	 * @return A scanner with the specified source and pattern
	 */
	private VisualScanner(Readable source, Pattern pattern) {
		if (source == null)
		...
		sourceDescription = source.getClass().getName();
	}
	 ...
	 */
	public VisualScanner(Readable source) {
	 ...
	 */
	public VisualScanner(InputStream source) {
	 ...
	 *            An input stream to be scanned
	 * @param charsetName
	 *            The encoding type used to convert bytes from the stream into
	 *            characters to be scanned
	 * @throws IllegalArgumentException
	 *             if the specified character set does not exist
	 */
	public VisualScanner(InputStream source, String charsetName) {
	...
	/**
	 * Constructs a new <code>Scanner</code> that produces values scanned from
	 * the specified file. Bytes from the file are converted into characters
	 * using the underlying platform's
	 * {@linkplain java.nio.charset.Charset#defaultCharset() default charset}.
	 * 
	 * @param source
	...
	public VisualScanner(File source) throws FileNotFoundException {
		this((ReadableByteChannel) (new FileInputStream(source).getChannel()));
		sourceDescription = "File(\"" + source.toString() + "\")";
	}
	 ...
	 */
	@SuppressWarnings("resource")
	public VisualScanner(File source, String charsetName) throws FileNotFoundException {
		this((ReadableByteChannel) (new FileInputStream(source).getChannel()), charsetName);
		sourceDescription = "File(\"" + source.toString() + "\"), charset: " + charsetName;
	}
	 ...
	 *            A string to scan
	 */
	public VisualScanner(String source) {
	...
	/**
	 * Constructs a new <code>Scanner</code> that produces values scanned from
	 * the specified channel. Bytes from the source are converted into
	 * characters using the underlying platform's
	 * {@linkplain java.nio.charset.Charset#defaultCharset() default charset}.
	 * 
	 * @param source
	 *            A channel to scan
	 */
	public VisualScanner(ReadableByteChannel source) {
	 ...
	 *            A channel to scan
	 * @param charsetName
	 *            The encoding type used to convert bytes from the channel into
	 *            characters to be scanned
	 * @throws IllegalArgumentException
	 *             if the specified character set does not exist
	 */
	public VisualScanner(ReadableByteChannel source, String charsetName) {
		this(makeReadable(source, charsetName), WHITESPACE_PATTERN);
	}
	 ...
	 * 
	 * A token is complete if surrounded by delims; a partial token is prefixed
	 * by delims but not postfixed by them
	 * 
	 * The position is advanced to the end of that complete token
	 * 
	 * Pattern == null means accept any token at all
	 * 
	 * Triple return: 1. valid string means it was found 2. null with
	 * needInput=false means we won't ever find it 3. null with needInput=true
	 * means try again after readInput
	 */
	 ...
	 * {@link java.io.Closeable} interface then the readable's <tt>close</tt>
	 * method will be invoked. If this scanner is already closed then invoking
	 ...
	 * @return this scanner
	 */
	public VisualScanner useDelimiter(Pattern pattern) {
		delimPattern = pattern;
		return this;
	}
	 ...
	 * 
	 * <p>
	 * An invocation of this method of the form <tt>useDelimiter(pattern)</tt>
	 * behaves in exactly the same way as the invocation
	 * <tt>useDelimiter(Pattern.compile(pattern))</tt>.
	 * 
	 * <p>
	 ...
	 * @return this scanner
	 */
	public VisualScanner useDelimiter(String pattern) {
	 ...
	 * matching regular expressions; see <a href= "#localized-numbers">localized
	 * numbers</a> above.
	 * 
	 ...
	 * @return this scanner
	 */
	public VisualScanner useLocale(Locale locale) {
	 ...
	 * expressions; see <a href= "#localized-numbers">localized numbers</a>
	 * above.
	 * 
	 ...
	 * 
	 * <p>
	 * If the radix is less than <code>Character.MIN_RADIX</code> or greater
	 * than <code>Character.MAX_RADIX</code>, then an
	 ...
	 */
	public VisualScanner useRadix(int radix) {
	 ...
	 * scanner. This method throws <code>IllegalStateException</code> if no
	 * match has been performed, or if the last match was not successful.
	 * 
	 * <p>
	 * The various <code>next</code>methods of <code>Scanner</code> make a match
	 * result available if they complete without throwing an exception. For
	 * instance, after an invocation of the {@link #nextInt} method that
	 * returned an int, this method returns a <code>MatchResult</code> for the
	 * search of the <a href="#Integer-regex"><i>Integer</i></a> regular
	 * expression defined above. Similarly the {@link #findInLine},
	 * {@link #findWithinHorizon}, and {@link #skip} methods will make a match
	...
	/**
	 * <p>
	 * Returns the string representation of this <code>Scanner</code>. The
	 * string representation of a <code>Scanner</code> contains information that
	 * may be useful for debugging. The exact format is unspecified.
	 * 
	 ...
	 * previous invocation of {@link #hasNext} returned <code>true</code>.
	 * 
	 ...
	 * 
	 * <p>
	 * An invocation of this method of the form <tt>hasNext(pattern)</tt>
	 * behaves in exactly the same way as the invocation
	 ...
	 * 
	 * <p>
	 * An invocation of this method of the form <tt>next(pattern)</tt> behaves
	...
	public boolean hasNext(Pattern pattern) {
		try {
			initiateMethodCall("hasNext(" + pattern + ")");
			ensureOpen();
			if (pattern == null)
	 ...
	 * of {@link #hasNext(Pattern)} returned <code>true</code>. If the match is
	...
	public String next(Pattern pattern) {
		try {
			initiateMethodCall("next(" + pattern + ")");
			ensureOpen();
			if (pattern == null)
				throw new NullPointerException();
	...
	public boolean hasNextLine() {
		try {
			initiateMethodCall("hasNextLine()");
			saveState();
	 ...
	 * 
	 * <p>
	 * An invocation of this method of the form <tt>findInLine(pattern)</tt>
	 * behaves in exactly the same way as the invocation
	 ...
	 * the next line separator, then <code>null</code> is returned and the
	 * scanner's position is unchanged. This method may block waiting for input
	...
	public String findInLine(Pattern pattern) {
		try {
			initiateMethodCall("findInLine(" + pattern + ")");
			ensureOpen();
			if (pattern == null)
				throw new NullPointerException();
			clearCaches();
	 ...
	 * 
	 * <p>
	 * A scanner will never search more than <code>horizon</code> code points
	 * beyond its current position. Note that a match may be clipped by the
	 * horizon; that is, an arbitrary match result may have been different if
	 * the horizon had been larger. The scanner treats the horizon as a
	 * transparent, non-anchoring bound (see
	 * {@link Matcher#useTransparentBounds} and
	 * {@link Matcher#useAnchoringBounds}).
	 * 
	 * <p>
	 * If horizon is <code>0</code>, then the horizon is ignored and this method
	...
	public String findWithinHorizon(Pattern pattern, int horizon) {
		try {
			initiateMethodCall("findWithinHorizon(" + pattern + ", " + horizon + ")");
			ensureOpen();
			if (pattern == null)
				throw new NullPointerException();
			if (horizon < 0)
	 ...
	 * then no input is skipped and a <tt>NoSuchElementException</tt> is thrown.
	 * 
	 * <p>
	 ...
	 * <code>NoSuchElementException</code> by using a pattern that can match
	 * nothing, e.g., <code>sc.skip("[ \t]*")</code>.
	 * 
	 * @param pattern
	 ...
	 *             if this scanner is closed
	 */
	public VisualScanner skip(Pattern pattern) {
		try {
			initiateMethodCall("skip(" + pattern + ")");
			ensureOpen();
			if (pattern == null)
				throw new NullPointerException();
			clearCaches();
	 ...
	 * 
	 * <p>
	 * An invocation of this method of the form <tt>skip(pattern)</tt> behaves
	 * in exactly the same way as the invocation
	 ...
	 *             if this scanner is closed
	 */
	public VisualScanner skip(String pattern) {
	...
	public boolean hasNextBoolean() {
		try {
			initiateMethodCall("hasNextBoolean()");
	 ...
	 * value. This method will throw <code>InputMismatchException</code> if the
	 * next token cannot be translated into a valid boolean value. If the match
	...
	public boolean nextBoolean() {
		try {
			initiateMethodCall("nextBoolean()");
			clearCaches();
	...
	public boolean hasNextByte() {
		try {
			initiateMethodCall("hasNextByte()");
	...
	public boolean hasNextByte(int radix) {
		try {
			initiateMethodCall("hasNextByte(" + radix + ")");
			setRadix(radix);
			boolean result = hasNext(integerPattern());
			if (result) { // Cache it
				try {
					String s = (matcher.group(SIMPLE_GROUP_INDEX) == null) ? processIntegerToken(hasNextResult)
							: hasNextResult;
	 ...
	 * 
	 * <p>
	 * An invocation of this method of the form <tt>nextByte()</tt> behaves in
	 * exactly the same way as the invocation <tt>nextByte(radix)</tt>, where
	 * <code>radix</code> is the default radix of this scanner.
	 * 
	 * @return the <tt>byte</tt> scanned from the input
	 * @throws InputMismatchException
	 *             if the next token does not match the <i>Integer</i> regular
	...
	/**
	 * Scans the next token of the input as a <tt>byte</tt>. This method will
	 * throw <code>InputMismatchException</code> if the next token cannot be
	 * translated into a valid byte value as described below. If the translation
	 * is successful, the scanner advances past the input that matched.
	 * 
	 * <p>
	 * If the next token matches the <a href="#Integer-regex"><i>Integer</i></a>
	 * regular expression defined above then the token is converted into a
	 * <tt>byte</tt> value as if by removing all locale specific prefixes, group
	 * separators, and locale specific suffixes, then mapping non-ASCII digits
	 * into ASCII digits via {@link Character#digit Character.digit}, prepending
	 * a negative sign (-) if the locale specific negative prefixes and suffixes
	...
	public byte nextByte(int radix) {
		try {
			initiateMethodCall("nextByte(" + radix + ")");
	...
	public boolean hasNextShort() {
		try {
			initiateMethodCall("hasNextShort()");
	...
	public boolean hasNextShort(int radix) {
		try {
			initiateMethodCall("hasNextShort(" + radix + ")");
			setRadix(radix);
			boolean result = hasNext(integerPattern());
			if (result) { // Cache it
				try {
	 ...
	 * 
	 * <p>
	 * An invocation of this method of the form <tt>nextShort()</tt> behaves in
	 * exactly the same way as the invocation <tt>nextShort(radix)</tt>, where
	 * <code>radix</code> is the default radix of this scanner.
	 * 
	...
	/**
	 * Scans the next token of the input as a <tt>short</tt>. This method will
	 * throw <code>InputMismatchException</code> if the next token cannot be
	 ...
	 * If the next token matches the <a href="#Integer-regex"><i>Integer</i></a>
	 * regular expression defined above then the token is converted into a
	 * <tt>short</tt> value as if by removing all locale specific prefixes,
	 * group separators, and locale specific suffixes, then mapping non-ASCII
	 * digits into ASCII digits via {@link Character#digit Character.digit},
	 * prepending a negative sign (-) if the locale specific negative prefixes
	 * and suffixes were present, and passing the resulting string to
	...
	public short nextShort(int radix) {
		try {
			initiateMethodCall("nextShort(" + radix + ")");
			// Check cached result
	...
	public boolean hasNextInt(int radix) {
		try {
			initiateMethodCall("hasNextInt(" + radix + ")");
			setRadix(radix);
			boolean result = hasNext(integerPattern());
			if (result) { // Cache it
				try {
	...
	/**
	 * The integer token must be stripped of prefixes, group separators, and
	 * suffixes, non ascii digits must be converted into ascii digits before
	 ...
	 * 
	 * <p>
	 * An invocation of this method of the form <tt>nextInt()</tt> behaves in
	 * exactly the same way as the invocation <tt>nextInt(radix)</tt>, where
	 * <code>radix</code> is the default radix of this scanner.
	 * 
	...
	/**
	 * Scans the next token of the input as an <tt>int</tt>. This method will
	 * throw <code>InputMismatchException</code> if the next token cannot be
	 ...
	 * <tt>int</tt> value as if by removing all locale specific prefixes, group
	 * separators, and locale specific suffixes, then mapping non-ASCII digits
	 * into ASCII digits via {@link Character#digit Character.digit}, prepending
	 * a negative sign (-) if the locale specific negative prefixes and suffixes
	 * were present, and passing the resulting string to
	...
	public int nextInt(int radix) {
		try {
			initiateMethodCall("nextInt(" + radix + ")");
			// Check cached result
			if ((typeCache != null) && (typeCache instanceof Integer) && this.radix == radix) {
	...
	public boolean hasNextLong() {
		try {
			initiateMethodCall("hasNextLong()");
	...
	public boolean hasNextLong(int radix) {
		try {
			initiateMethodCall("hasNextLong(" + radix + ")");
			setRadix(radix);
			boolean result = hasNext(integerPattern());
			if (result) { // Cache it
				try {
	 ...
	 * 
	 * <p>
	 * An invocation of this method of the form <tt>nextLong()</tt> behaves in
	 * exactly the same way as the invocation <tt>nextLong(radix)</tt>, where
	 * <code>radix</code> is the default radix of this scanner.
	 * 
	...
	/**
	 * Scans the next token of the input as a <tt>long</tt>. This method will
	 * throw <code>InputMismatchException</code> if the next token cannot be
	 ...
	 * If the next token matches the <a href="#Integer-regex"><i>Integer</i></a>
	 * regular expression defined above then the token is converted into a
	 * <tt>long</tt> value as if by removing all locale specific prefixes, group
	 * separators, and locale specific suffixes, then mapping non-ASCII digits
	 * into ASCII digits via {@link Character#digit Character.digit}, prepending
	 * a negative sign (-) if the locale specific negative prefixes and suffixes
	 * were present, and passing the resulting string to
	...
	public long nextLong(int radix) {
		try {
			initiateMethodCall("nextLong(" + radix + ")");
			// Check cached result
	...
	/**
	 * The float token must be stripped of prefixes, group separators, and
	 * suffixes, non ascii digits must be converted into ascii digits before
	 * parseFloat will accept it.
	 * 
	 * If there are non-ascii digits in the token these digits must be processed
	...
	public boolean hasNextFloat() {
		try {
			initiateMethodCall("hasNextFloat()");
			setRadix(10);
	...
	/**
	 * Scans the next token of the input as a <tt>float</tt>. This method will
	 * throw <code>InputMismatchException</code> if the next token cannot be
	 ...
	 * 
	 * <p>
	 * If the next token matches the <a href="#Float-regex"><i>Float</i></a>
	 * regular expression defined above then the token is converted into a
	 * <tt>float</tt> value as if by removing all locale specific prefixes,
	 * group separators, and locale specific suffixes, then mapping non-ASCII
	 * digits into ASCII digits via {@link Character#digit Character.digit},
	 * prepending a negative sign (-) if the locale specific negative prefixes
	 * and suffixes were present, and passing the resulting string to
	 ...
	 * @throws InputMismatchException
	 *             if the next token does not match the <i>Float</i> regular
	 *             expression, or is out of range
	 * @throws NoSuchElementException
	 *             if input is exhausted
	 * @throws IllegalStateException
	...
	public boolean hasNextDouble() {
		try {
			initiateMethodCall("hasNextDouble()");
			setRadix(10);
			boolean result = hasNext(floatPattern());
			if (result) { // Cache it
				try {
	...
	/**
	 * Scans the next token of the input as a <tt>double</tt>. This method will
	 * throw <code>InputMismatchException</code> if the next token cannot be
	 ...
	 * If the next token matches the <a href="#Float-regex"><i>Float</i></a>
	 * regular expression defined above then the token is converted into a
	 * <tt>double</tt> value as if by removing all locale specific prefixes,
	 * group separators, and locale specific suffixes, then mapping non-ASCII
	 * digits into ASCII digits via {@link Character#digit Character.digit},
	 * prepending a negative sign (-) if the locale specific negative prefixes
	 * and suffixes were present, and passing the resulting string to
	...
	/**
	 * Returns true if the next token in this scanner's input can be interpreted
	 * as a <code>BigInteger</code> in the default radix using the
	...
	public boolean hasNextBigInteger() {
		try {
			initiateMethodCall("hasNextBigInteger()");
	...
	/**
	 * Returns true if the next token in this scanner's input can be interpreted
	 * as a <code>BigInteger</code> in the specified radix using the
	 * {@link #nextBigInteger} method. The scanner does not advance past any
	 * input.
	 * 
	 * @param radix
	...
	public boolean hasNextBigInteger(int radix) {
		try {
			initiateMethodCall("hasNextBigInteger(" + radix + ")");
			setRadix(radix);
			boolean result = hasNext(integerPattern());
			if (result) { // Cache it
				try {
	 ...
	 * 
	 * <p>
	 * An invocation of this method of the form <tt>nextBigInteger()</tt>
	 * behaves in exactly the same way as the invocation
	 * <tt>nextBigInteger(radix)</tt>, where <code>radix</code> is the default
	 * radix of this scanner.
	 * 
	...
	public BigInteger nextBigInteger() {
		try {
			initiateMethodCall("nextBigInteger()");
	 ...
	 * If the next token matches the <a href="#Integer-regex"><i>Integer</i></a>
	 * regular expression defined above then the token is converted into a
	 * <tt>BigInteger</tt> value as if by removing all group separators, mapping
	 * non-ASCII digits into ASCII digits via the {@link Character#digit
	 * Character.digit}, and passing the resulting string to the
	 * {@link java.math.BigInteger#BigInteger(java.lang.String)
	...
	public BigInteger nextBigInteger(int radix) {
		try {
			initiateMethodCall("nextBigInteger(" + radix + ")");
			// Check cached result
	...
	/**
	 * Returns true if the next token in this scanner's input can be interpreted
	 * as a <code>BigDecimal</code> using the {@link #nextBigDecimal} method.
	 * The scanner does not advance past any input.
	 * 
	 * @return true if and only if this scanner's next token is a valid
	...
	public boolean hasNextBigDecimal() {
		try {
			initiateMethodCall("hasNextBigDecimal()");
			setRadix(10);
	 ...
	 * 
	 * <p>
	 * If the next token matches the <a href="#Decimal-regex"><i>Decimal</i></a>
	 * regular expression defined above then the token is converted into a
	 * <tt>BigDecimal</tt> value as if by removing all group separators, mapping
	 * non-ASCII digits into ASCII digits via the {@link Character#digit
	 * Character.digit}, and passing the resulting string to the
	 * {@link java.math.BigDecimal#BigDecimal(java.lang.String)
	 * BigDecimal(String)} constructor.
	 * 
	 * @return the <tt>BigDecimal</tt> scanned from the input
	 * @throws InputMismatchException
	 *             if the next token does not match the <i>Decimal</i> regular
	 *             expression, or is out of range
	 * @throws NoSuchElementException
	 *             if the input is exhausted
	 * @throws IllegalStateException
	...
	public BigDecimal nextBigDecimal() {
		try {
			initiateMethodCall("nextBigDecimal()");
			// Check cached result
	 ...
	 * may have been changed by invocations of {@link #useDelimiter},
	 * {@link #useLocale}, or {@link #useRadix}.
	 * 
	 * <p>
	 * An invocation of this method of the form <tt>scanner.reset()</tt> behaves
	 * in exactly the same way as the invocation
	 * 
	 * <blockquote>
	 * 
	 ...
	 * @since 1.6
	 */
	public VisualScanner reset() {
		try {
			initiateMethodCall("reset()");
	...
	// ----------------------------------------------------------------------
	// Instance variables and methods for visualizing the current state of
	// this scanner.
	// ----------------------------------------------------------------------

	// Frame displaying internal content of this scanner
	private volatile ScannerFrame frame;

	// Number of methods which were started but not finalized.
	private int openMethodsCount = 0;

	// Indicates that all calls that will be executed are private from
	// ScannerFrame
	private boolean privateCalls = false;

	// Wait lock used to block the current thread until the user permits to
	// continue
	final Object waitLock = new Object();

	// Description of the scanner source
	volatile String sourceDescription;

	// Name of operation (method) that will executed after confirmation
	volatile String currentOperation;

	/**
	 * Initializes an execution of a scanner method. This methods is called at
	 * the beginning of each public method of the scanner that is related to
	 * scanning the content.
	 * 
	 * @param method
	 *            name of the called method
	 */
	private void initiateMethodCall(String method) {
		// if this is a call that is a result of visualization actions, we do
		// nothing
		if (privateCalls)
			return;
		...
		// if this is the first "public" call, we show the window and update its
		// state
		if (openMethodsCount == 1) {
			currentOperation = method;
			updateVisualizationFrame();
			// wait for confirmation of the user to execute this method
			waitForPermission();
		}
	}

	/**
	 * Finalizes an execution of a scanner method. This methods is called in the
	 * finally block of each public method of the scanner that is related to
	 * scanning the content.
	 */
	private void finalizeMethodCall() {
		// if this is a call that is a result of visualization actions, we do
		// nothing
		if (privateCalls)
			return;
		...
		// if number of finalization reached 0, we update the visualization
		// frame
		if (openMethodsCount == 0) {
			currentOperation = null;
			if (EventQueue.isDispatchThread()) {
				throw new RuntimeException("Methods of VisualScanner cannot be called from the Event Dispatch Thread.");
			}
			updateVisualizationFrame();
		}
	}
	...
	/**
	 * Block the current thread until an action is confirmed by the user in the
	 * visualization frame.
	 */
	...
	private void updateVisualizationFrame() {
		// set that all next method invocations of Scanner are private (not
		// realized by the user code)
		privateCalls = true;

		// realized the update of the visualization frame in the EDT
		try {
			EventQueue.invokeAndWait(new Runnable() {
				public void run() {
					// if the scanner is closed, we close the frame
					if (closed) {
						if (frame != null) {
					...
					// create a new frame, if this is the first operation
					if (frame == null) {
						try {
							UIManager.setLookAndFeel(UIManager.getSystemLookAndFeelClassName());
						} catch (Exception ignore) {
						}
						frame = new ScannerFrame(VisualScanner.this);
		...
		// set that all next method invocations are caused by the user code
		privateCalls = false;
	}

	/**
	 * Returns a next token to be scanned by the scanner.
	 */
	String getNextToken() {
		if (!hasNext()) {
			return null;
	...
	/**
	 * Returns the unscanned content in the buffer of this scanner.
	 */
	String getBuffer() {
		if (buf != null) {
			return buf.subSequence(position, buf.limit()).toString();
		} else {
			return null;
		}
	}
